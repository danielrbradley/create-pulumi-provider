...
(async function () {
  try {
    ...
    const writeString = (filePath: string, template: string) =>
      fs.writeFileSync(path.join(name, filePath), template.trimStart());
    const writeJson = (filePath: string, data: any) =>
      writeString(filePath, JSON.stringify(data, undefined, 2));
